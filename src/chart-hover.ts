...
export function placeReadings<T extends { y: number }>(
  readings: T[],
  top: number,
  bottom: number,
  gap = 23,
) {
  const sorted = readings.map((item) => ({ ...item, labelY: 0 })).sort((a, b) => a.y - b.y);
  sorted.forEach((item, index) => {
    item.labelY = Math.max(top, item.y, index ? sorted[index - 1].labelY + gap : top);
  });
  for (let index = sorted.length - 1; index >= 0; index--) {
    sorted[index].labelY = Math.min(
      sorted[index].labelY,
      index === sorted.length - 1 ? bottom : sorted[index + 1].labelY - gap,
    );
  }
  return sorted;
}

export function hoverMarkup({
  point,
  hz,
  db,
  readings,
  ...
  const badge = (x: number, y: number, width: number, text: string, className = '') =>
    /* HTML */ `<g class="hover-badge ${className}" transform="translate(${x},${y})"
      ><rect x="${-width / 2}" y="-11" width="${width}" height="22" rx="4" /><text
        text-anchor="middle"
        y="4"
        >${text}</text
      ></g
  ...
  let html = /* HTML */ `<path
      ...
      d="M${point.x} ${t}V${b}M${l} ${point.y}H${r}"
    /><circle class="hover-pointer" cx="${point.x}" cy="${point.y}" r="2.5" />`;
  ...
  for (const item of placeReadings(items, t + 13, b - 15)) {
    ...
    html += /* HTML */ `<g class="hover-reading" style="--reading-color:${item.color}"
      ><path
        class="hover-leader"
        d="M${point.x} ${item.y}L${textX + (leftward ? -5 : 5)} ${item.labelY}"
      /><circle
        cx="${point.x}"
        cy="${item.y}"
        r="3.5"
        class="hover-curve-dot"
        ${offscale ? ' stroke-dasharray="2 2"' : ''}
