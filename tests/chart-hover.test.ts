...
import { placeReadings, hoverMarkup } from '../src/chart-hover.ts';

test('overlapping curve readings stay ordered, separated, and inside the plot', () => {
  for (const center of [24, 200, 447]) {
    const result = placeReadings(
      Array.from({ length: 5 }, (_, i) => ({ y: center + i })),
      37,
      432,
    );
...
test('hover shows precise cursor values and marks offscreen curve values', () => {
  const markup = hoverMarkup({
    point: { x: 1100, y: 200 },
    hz: 1234,
    db: -3.25,
    readings: [{ name: 'Source', color: '#dc6576', db: 40 }],
    bounds: { l: 54, r: 1175, t: 24, b: 447 },
    yOf: (db) => 235 - db * 8,
  });
